...
class Stat {
  ...
  getComboText() {
    return this.combo + (this.comboHTPreserve ? "!" : "");
  }

  getData(isAuto, speed = "", app) {
...
class Renderer {
  ...
  applyJDMModifier(jdmIn) {
    ...
    if (this.mods.has("hr")) {
      jdmIn.touch /= 1.2; // Increase bad section a bit
      jdmIn.good /= 2;
      jdmIn.perfect /= 2;
      jdmIn.rainbow /= 2;
    } else if (this.mods.has("ez")) {
    ...
    } else if (this.mods.has("rl")) {
      jdmIn.rainbow = jdmIn.perfect = jdmIn.good = jdmIn.touch;
    }
  }
